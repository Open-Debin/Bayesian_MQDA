...
# CUDA_VISIBLE_DEVICES=1 python ur-transformer-single-henry.py -title 1prior_adam_l2n_henry_short --reg_param 0.95
def main():
    args = load_config()
    ...
    optimizer = config.optimizer(chain(params, learner.parameters()), args)    
...
#         Group ___B
        # Group A___
        urt_state, learner, args, optimizer_state, lr_schedule_state = load.urt_mqda(ckps[0])
...
#     args.pca_v_matrix = torch.load('PCA_S_V_10.pth')[1].cuda()
    index = train_mqda(args, train_loader, URT_model, learner, logger, 'meta_train', start_iter, optimizer, lr_scheduler)
#     test_all_dataset_mqda(args, val_loaders,  classifier, logger, "meta_eval", index)
    test_all_dataset_mqda(args, test_loaders, URT_model, learner, logger, "meta_test", index)

def test_all_dataset_mqda(args, test_loaders, URT_model, learner, logger, mode, training_iter):
    URT_model.eval()
    learner.eval()
    our_name = 'mqda'
    accs_names = [our_name]
    ...
    with torch.no_grad():
        for dataset_id, (test_dataset, loader) in enumerate(test_loaders.items()):
            ...
            for episode_index, (_, x_spt, y_spt, x_qry, y_qry) in enumerate(loader):
                ...
                predictor = learner.fit(urt_x_spt, y_spt) # compute \mu and \Sigma on support set
                logits = predictor.predict(urt_x_qry)
...
def train_mqda(args, train_loader, URT_model, learner, logger, mode, start_iter, optimizer, lr_scheduler):
    URT_model.train()
    learner.train()
...
#         break
        ...
        predictor = learner.fit(urt_x_spt, y_spt) # compute \mu and \Sigma on support set
        logits = predictor.predict(urt_x_qry) 
        reg_term = learner._hyperprior(learner.S_lower, args.cholesky_alpha)
#         croent_loss_spt = (F.cross_entropy(outputs_spt, y_spt) - args.loss_threshold_spt).abs() # + args.loss_threshold_spt                
        loss = F.cross_entropy(logits, y_qry) + reg_term
        
        "===>MQDA"
        losses.backward_propagation(loss, learner.parameters(), optimizer)
        lr_scheduler.step()
        ...
        if episode_index % args.n_episode_train == 0 or episode_index+1 == args.n_epoch * args.n_episode_train or episode_index == 10:
            ...
            save.urt_mqda(args, episode_index, optimizer.state_dict(), lr_scheduler.state_dict(), URT_model.state_dict(), learner.state_dict(), args.save_dir, 'train')
            our_losses.reset()
...
def load_config():
    ...
    parser.add_argument('--n_epoch', type=int, default=20, help='The number to log training information')
    parser.add_argument('--n_episode_train', default=500, type=int, help='number of episode to train (default: 10000)')
...
#     parser.add_argument('--Nj', default=1, type=int, help='')
    ...
    # mqda related
    parser.add_argument('--prior_initial',default='', help="")
    parser.add_argument('--reject_prior',default=0.0, help="")
    parser.add_argument('--loss_threshold_spt', default=0.32, type=float, help='(loss-t).abs()+t')
    parser.add_argument('--cholesky_alpha',default=0, type=float,help="hope cholesky_alpha > feature dim + 1 ")
    parser.add_argument('-strategy','--strategy', default='fb_openset', type=str, help='')
